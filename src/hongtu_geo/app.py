...
const ar=state.ai_probe_readiness;document.getElementById('aiProbeReadiness').innerHTML=ar.providers.length?ar.providers.map(p=>{const retry=p.retry_state==='retry_due'?'限制已到期，后台可恢复尝试':p.retry_state==='restriction_active'?`限制中，${p.retry_after} 后重试`:p.retry_state==='invalid'||p.retry_state==='unknown'?'重试时间无效，保持暂停':'';return `<tr><td><span class="pill">${esc(ar.status)}</span></td><td>${esc(p.name)} (${esc(p.provider)})</td><td>${p.adapter_valid?'有效':'无效'}</td><td>${esc(p.effective_account_status||p.account_status)}</td><td>${p.ready_for_attempt?'是':'否'}</td><td>${esc(p.last_checked_at||'未检查')}</td><td>${esc(p.configuration_errors.join('；')||retry||(p.last_error_present?'存在最近错误；请查看账号详情':ar.method_note))}</td></tr>`}).join(''):`<tr><td colspan="7" class="empty">尚未配置 AI 监测适配器</td></tr>`;
