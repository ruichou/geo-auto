...
<main>
  ...
  <section><div class="section-title"><h2>多周期趋势</h2><small>相邻窗口均达到样本门槛后才计算变化</small></div><div class="card table-card"><table><thead><tr><th>引擎 / surface</th><th>状态</th><th>当前样本</th><th>前期样本</th><th>提及变化</th><th>推荐变化</th><th>引用变化</th><th>区间信号</th></tr></thead><tbody id="trendRows"></tbody></table></div></section>
...
const t=state.trends;document.getElementById('trendRows').innerHTML=t.comparisons.length?t.comparisons.map(r=>{const d=r.deltas_percentage_points;const fmt=v=>v==null?'样本不足':`${v>0?'+':''}${v}pp`;return `<tr><td>${esc(r.provider)} / ${esc(r.engine_surface)} / ${esc(r.prompt_variant)} / ${esc(r.prompt_version)}</td><td><span class="pill">${esc(r.status)}</span></td><td>${r.current.samples}</td><td>${r.previous.samples}</td><td>${fmt(d.mention_rate)}</td><td>${fmt(d.recommendation_rate)}</td><td>${fmt(d.owned_citation_rate)}</td><td>${esc(r.non_overlapping_ci_signals.join('、')||'无')}</td></tr>`}).join(''):`<tr><td colspan="8" class="empty">尚无可比较的真实样本</td></tr>`;
